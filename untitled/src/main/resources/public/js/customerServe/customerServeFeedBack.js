layui.use(['table','layer'],function() {
    ...
    var tableIns = table.render({
        ...
        url : 'customer_serve/list?state=fw_003&flag=1', // 访问数据的地址
        cellMinWidth : 95,
        page : true, // 开启分页
        height : "full-125",
        limits : [10,15,20,25],
    ...
    table.on('tool(customerServes)', function(obj){
        if(obj.event == "feedBack"){
            openCCustomerServeProceDialog("<h3>服务反馈</h3>","customer_serve/toCustomerServeFeedBackPage?id=" + obj.data.id);
        }
    });

    // 开启新窗口
    function openCCustomerServeProceDialog(title, url) {
        title = "<h2>" + title + "</h2>";
        layui.layer.open({
            type: 2,
            title: title,
