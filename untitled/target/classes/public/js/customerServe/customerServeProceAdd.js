layui.use(['form', 'layer'], function () {
    ...
    $.post(
        crm + "/user/queryAllCustomerManagers",
        {},
        function (data) {
            $.each(data, function (index, element) {
                ...
                if (element.id == assignMan) {
                    $("#assigner").append(new Option(element.uname, element.id, false,true));
                } else {
                    $("#assigner").append(new Option(element.uname, element.id));
                }
            });
            // 必须重新渲染下拉框
            form.render("select");
